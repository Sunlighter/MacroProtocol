...
namespace Sunlighter.MacroProtocol.Sockets
{
    public static class SocketExtensions
    {
        ...
        public static async Task SendAsync<T>(this ITypeTraits<T> softType, Socket s, T value)
        {
            byte[] block = softType.SerializeToBytes(value);
            await s.SendBlock(block);
        }

        public static async Task SendAsync<T>(this ITypeTraits<T> softType, Socket s, T value, CancellationToken cToken)
        {
            byte[] block = softType.SerializeToBytes(value);
            await s.SendBlock(block, cToken);
        }

        public static async Task<Option<T>> ReceiveAsync<T>(this ITypeTraits<T> softType, Socket s)
        {
            Option<byte[]> block = await s.ReceiveBlock();
            if (block.HasValue)
                return Option<T>.Some(softType.DeserializeFromBytes(block.Value));
            else
                return Option<T>.None;
        }

        public static async Task<Option<T>> ReceiveAsync<T>(this ITypeTraits<T> softType, Socket s,  CancellationToken cToken)
        {
